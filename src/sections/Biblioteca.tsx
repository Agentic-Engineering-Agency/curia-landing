...
const cards = [
  ...
  },
  {
    ...
    body: "Los archivos, su visualización, el estado de procesamiento y el historial de consulta permanecen ligados al caso correspondiente.",
  },
  {
    icon: ListChecks,
      ...
      <div className="curia-shell">
        <div className="max-w-4xl">
          ...
          <p className="mt-6 max-w-3xl text-base leading-7 text-[var(--curia-text-secondary)] md:text-lg md:leading-8">
            Cada expediente cuenta con una Biblioteca para cargar, visualizar y descargar documentos. El análisis parte de los archivos del caso, no de una conversación desconectada del asunto.
          </p>
        </div>

        <div className="mt-12 grid items-start gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10">
          <div className="curia-card-editorial overflow-hidden lg:sticky lg:top-28">
            <div className="border-b border-[var(--curia-border)] bg-[var(--curia-bg-subtle)] px-5 py-4 md:px-6">
              <p className="text-sm font-semibold">Archivos del expediente</p>
              ...
              {documents.map((document) => (
                <li key={document.name} className="flex items-center gap-3 px-5 py-4 md:px-6">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 inline-flex rounded-md border border-[var(--curia-border)] bg-white px-1.5 py-0.5 text-[0.65rem] font-bold tracking-[0.05em] text-[var(--curia-text-secondary)]">
                      {document.format}
                    </span>
                  </div>
                  <span
                    className={`shrink-0 rounded-full px-2.5 py-1 text-[0.68rem] font-semibold ${statusClass[document.status]}`}
                  >
                    {document.status}
                  </span>
                </li>
